...
class RelatedPostsThumbnails {
	...
	function get_thumbnails( $show_top = false ) { // Retrieve Related Posts HTML for output
		...
		foreach( $posts as $post ) {
			...
			$tit = $this->process_text_cut( $post->post_title, $text_length );
			...
			if ( !empty( $tit ) && !empty( $price ) ) {
				$title = '<div class="price_related_posts_thumbnails"><b>'.$price . ' РУБ' . '</b></div>';
				$excerpt = '<b>' . $tit . '</b>'.'<br/>' . $locations[0]->name ;
